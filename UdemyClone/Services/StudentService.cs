...
namespace UdemyClone.Services
{
    public class StudentService : IStudentService
    {
        ...
        public async Task<LessonModel> GetLessonAsync(Guid studentId, Guid lessonId)
        {
            ...
            if (lesson.Quizzes.Any())
            {
                foreach (var quiz in lesson.Quizzes)
                {
                    var quizResult = await context.StudentQuizzes
                                            .Where(sq => sq.StudentId == studentId && sq.QuizId == quiz.Id)
        ...
        public async Task<IEnumerable<StudentDto>> GetAllStudentsAsync(int pageNumber, int pageSize)
        {
            ...
            return await (
                            ...
                            select new StudentDto
                            {
                                ...
                                Email = user.Email,
                                Enrollments = user.Enrollments.Select (
                                                                         e => new EnrollmentDto
                                                                            {
                                                                                CourseId = e.CourseId,
